    ...
    if (isset($_POST['upload_flower'])) {
        ...
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
            try {
                ...
                // Request both LABEL_DETECTION and WEB_DETECTION for higher accuracy
                $features = [
                    (new Feature())->setType(Type::LABEL_DETECTION),
                    (new Feature())->setType(Type::WEB_DETECTION)
                ];
                ...
                $res = $response->getResponses()[0];

                // --- STEP 1: Define broad set of flower keywords ---
                $flower_keywords = [
                    'flower', 'flowers', 'floral', 'bloom', 'blossom', 'plant', 'petal',
                    'flora', 'wildflower', 'inflorescence', 'ornamental plant', 'botany',
                    'rose', 'daisy', 'tulip', 'lily', 'sunflower', 'hibiscus', 'orchid',
                    'marigold', 'poppy', 'lotus', 'camellia', 'carnation', 'chrysanthemum',
                    'bouquet', 'garden plant', 'blossoming', 'floret'
                ];

                // --- STEP 2: Check LABEL_DETECTION results ---
                if ($res->getLabelAnnotations()) {
                    foreach ($res->getLabelAnnotations() as $label) {
                        $desc = strtolower($label->getDescription());
                        $score = $label->getScore(); // 0 to 1

                        foreach ($flower_keywords as $keyword) {
                            if (str_contains($desc, $keyword)) {
                                // Multiply by 1.0 weight (labels are strong indicators)
                                $flower_confidence += $score * 1.0;
                            }
                        }
                    }
                }

                // --- STEP 3: Check WEB_DETECTION results (best guesses + entities) ---
                if ($res->getWebDetection()) {
                    $web = $res->getWebDetection();

                    // Best guess labels (e.g., “Hibiscus flower”, “Sunflower close-up”)
                    foreach ($web->getBestGuessLabels() as $guess) {
                        $guessLabel = strtolower($guess->getLabel());
                        foreach ($flower_keywords as $keyword) {
                            if (str_contains($guessLabel, $keyword)) {
                                $flower_confidence += 0.5; // lighter weight
                            }
                        }
                    }

                    // Web entities (e.g., scientific names, common names)
                    foreach ($web->getWebEntities() as $entity) {
                        $desc = strtolower($entity->getDescription());
                        $score = $entity->getScore(); // 0 to 1 (if available)
                        foreach ($flower_keywords as $keyword) {
                            if (str_contains($desc, $keyword)) {
                                $flower_confidence += $score * 0.8; // medium weight
                            }
                        }
                    }
                }

                // --- STEP 4: Determine if it's a flower ---
                // The higher the confidence, the more likely it is to be a flower.
                // Normal flowers should reach ~1.0 or higher with this scoring.
                if ($flower_confidence >= 0.8) {
                    $is_a_flower = true;
                }

                if (!$is_a_flower) {
                    $upload_errors[] = sprintf(
                        "AI Rejected: This image does not appear to be a flower. (Confidence %.2f)",
                        $flower_confidence
                    );
                }
                ...
                $upload_errors[] = "General Error: " . $e->getMessage();
            }
        } else {
    ...
    if (isset($_POST['identify_flower'])) {
        if (isset($_FILES['identify_photo']) && $_FILES['identify_photo']['error'] === UPLOAD_ERR_OK) {
            ...
            try {
                ...
                $labels = $webDetection->getBestGuessLabels();
                ...
                if ($labels) {
                    ...
                    foreach ($labels as $label) {
                        $guess = $label->getLabel(); // e.g., "alstroemeria peruvian lily"
                        ...
                        if (!in_array(strtolower($guess), $blacklist)) {
                            ...
                            $weight = 1; 
                            if ($loop_count == 1) $weight = 5; // Top guess is most important
                            if ($loop_count == 2) $weight = 3; // Second guess is important
